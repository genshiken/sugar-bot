...
export class PlayMusicCommand extends Command {
    ...
    public async messageRun(message: Message, args: Args) {
        ...
        if (
            (searchRes?.loadType as LavalinkLoadType) === "LOAD_FAILED" ||
            !searchRes
        ) {
            await message.channel.send(
                "Failed to search that query. Try with different formatting, I guess?"
            );
            return;
        }
        ...
        if (!musicGuildInfo) {
            ...
            player.on("end", async (data) => {
                ...
                const currentMusicGuildInfo = musicManager.get(
                    message.guildId!
                );
                ...
                if (newMusicGuildInfo.isSkippingQueued) {
                    ...
                    newMusicGuildInfo.currentPosition =
                        newMusicGuildInfo.skipPosition;
                } else {
                ...
                if (
                    newMusicGuildInfo.currentPosition ===
                    newMusicGuildInfo.queue.length
                ) {
                    ...
                    if (newMusicGuildInfo.isRepeat === "playlist") {
                        ...
                        await message.channel.send(
                            "Playlist loop is set. Resetting playhead to the beginning of the queue."
                        );
                        const poppedTrack =
                            newMusicGuildInfo.queue[
                                newMusicGuildInfo.currentPosition
                            ]!;
        ...
        switch (searchRes.loadType as LavalinkLoadType) {
            case "TRACK_LOADED":
                ...
                await message.channel.send(
                    `Track loaded. ${
                        searchRes.tracks[0]?.info.title
                    } | Duration: ${fancyTimeFormat(
                        searchRes.tracks[0]?.info.length! / 1000
                    )} | Pos: ${
                        musicGuildInfo.queue.length
                    }. | Timestamp: ${fancyTimeFormat(
                        searchRes.tracks[0]!.info.position / 1000
                    )}`
                );
                break;
            ...
            case "SEARCH_RESULT":
                ...
                await message.channel.send(
                    `Track loaded. **${
                        ...
                        searchRes.tracks[0]?.info.length! / 1000
                    )} | Pos: ${musicGuildInfo.queue.length}`
                );
                break;
            case "NO_MATCHES":
